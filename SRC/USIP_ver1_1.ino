...
// USIP Copyright 2019 NASA, University of Bridgeport, Discovery Museum Bridgeport, CT.
// All rights reserved.
...
//String BIT_Str = ":AB1JC-11 :BITS.11111111, USIP Discovery3 Balloon\n";

// NEW for USIP ===>GPSok TimeSync COMMSok AltCAL MonkeyOK PreFlight Rising Falling
...
//
  if ((CMD_Str.length() > 6 ) && (CMD_Str.length() < 12)) {
    ...
    TSTStr.toUpperCase();
      ...
      if (TSTStr == "$ABORT") {      //Decode $ABORT message here. If not goto next else_if below.
        ...
        IDENT_flg = true;
        Serial.println("USIP Discovery Museum High Altitude Balloon");
      }    
 ...
 /// Monkey Commands from Computer      
        ...
        if (monkey_cmd >= 0 && monkey_cmd <= 84) {
          MonkeyStr = TSTStr + " " + CMD_Str.substring(7,9); // ex. MONKEY 07
...
//////////////////////////////////////////////////////////////////////////////
 ...
 void Process_IDENT()  {
     ...
     Serial1.println(" USIP Discovery3 Balloon");
...
#ifdef DEBUG 
      LCD_Clear();
      Serial3.print("Process Monkey  Reply");
#endif
...
void loop() {
  ...
  if (MONKEYCMD_flg) Process_MONKEYCMD();
...
//      Pressure_alt = 100 *3.281;                  commented out 11/18/19 LJR
    ...
    if (BIT_flg) {
      Serial1.print(F(":AB1JC-11 :BITS.11111111, USIP Discovery3 Balloon\n"));
      BIT_flg = false;
    }
...
//                 GPS_valid time_sync COMMSok Alt_CAL MonkeyOK  prelaunch rising falling
    ...
    if (COMMSok) {Serial1.print(ONE_Str);}
    else {Serial1.print(ZERO_Str);}
    if (Alt_Cal) {Serial1.print(ONE_Str);}
    else {Serial1.print(ZERO_Str);}
    if (MonkeyOK) {Serial1.print(ONE_Str);}
    else {Serial1.print(ZERO_Str);}
    if (prelaunch) {Serial1.print(ONE_Str);}
    else {Serial1.print(ZERO_Str);}
    if (rising) {Serial1.print(ONE_Str);}
    else {Serial1.print(ZERO_Str);}
    if (falling) {Serial1.println(ONE_Str);}
    else {Serial1.println(ZERO_Str);}
//////////////////////////////////////////
...
//                 GPS_valid time_sync COMMSok Alt_CAL MonkeyOK  prelaunch rising falling
    ...
    if (COMMSok) {Serial.print(ONE_Str);}
    else {Serial.print(ZERO_Str);}
    if (Alt_Cal) {Serial.print(ONE_Str);}
    else {Serial.print(ZERO_Str);}
    if (MonkeyOK) {Serial.print(ONE_Str);}
    else {Serial.print(ZERO_Str);}
    if (prelaunch) {Serial.print(ONE_Str);}
    else {Serial.print(ZERO_Str);}
    if (rising) {Serial.print(ONE_Str);}
    else {Serial.print(ZERO_Str);}
    if (falling) {Serial.println(ONE_Str);}
    else {Serial.println(ZERO_Str);}
  }
